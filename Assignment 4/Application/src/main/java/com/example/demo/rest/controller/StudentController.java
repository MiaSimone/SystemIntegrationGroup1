...
public class StudentController {
    ...
    private DBConnection storage = new DBConnection(conStr, user, pass);
//    @GetMapping("/")
...
//    }
    @GetMapping("/{id}")
    public EntityModel<Student> retrieveStudent(@PathVariable long id) throws SQLException {
        EntityModel<Student> resource = EntityModel.of(storage.getStudentWithId(id));
        return resource;
    }
...
//    }
    ...
    @PostMapping("/")
    public ResponseEntity<Object> saveStudent(@Valid @RequestBody Student student) throws SQLException {
        int savedStudentId = storage.createStudent(student);
        URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
                .buildAndExpand(savedStudentId).toUri();
        return ResponseEntity.created(location).build();
    }
